      ...
      <scroll class="shortcut" :data="shortcut" ref="shortcut" :refreshDelay="refreshDelay">
        <div>
          <div class="hot-key">
            ...
            <ul>
              <li v-for="(item,index) in hoyKey" :key="index" class="item" @click="addQuery(item.k)">
                <span>{{item.k}}</span>
              </li>
            </ul>
          </div>
...
export default {
  ...
  computed: {
    shortcut() {
      return this.hoyKey.concat(this.searchHistory)
    }
    // ...mapGetters([
    //   'searchHistory'
    // ])
  ...
  methods: {
    ...
    _getHotKey() {
      getHotKey().then((res) => {
        if (res.code === ERR_OK) {
          this.hoyKey = res.data.hotkey.slice(0, 10)
        }
      })
    },
    ...mapActions([
